...
def upgrade() -> None:
    ...
    _safe_add_constraint(
        conn, "jobs", "ALTER TABLE jobs ADD CONSTRAINT ck_jobs_progress CHECK (progress >= 0 AND progress <= 100)"
    )
    # File sizes must be non-negative
    _safe_add_constraint(conn, "images", "ALTER TABLE images ADD CONSTRAINT ck_images_file_size CHECK (file_size >= 0)")
    _safe_add_constraint(
        conn, "goes_frames", "ALTER TABLE goes_frames ADD CONSTRAINT ck_goes_frames_file_size CHECK (file_size >= 0)"
    )
    _safe_add_constraint(
        conn, "animations", "ALTER TABLE animations ADD CONSTRAINT ck_animations_file_size CHECK (file_size >= 0)"
    )
    _safe_add_constraint(
        conn, "composites", "ALTER TABLE composites ADD CONSTRAINT ck_composites_file_size CHECK (file_size >= 0)"
    )
    ...
    _safe_add_constraint(
        conn, "cleanup_rules", "ALTER TABLE cleanup_rules ADD CONSTRAINT ck_cleanup_rules_value CHECK (value > 0)"
    )
