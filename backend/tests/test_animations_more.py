...
async def test_crop_preset_crud(client):
    ...
    # Update
    resp = await client.put(f"/api/satellite/crop-presets/{pid}", json={"name": "4K"})
    assert resp.status_code == 200
...
async def test_animations_pagination(client):
    resp = await client.get("/api/satellite/animations?page=1&limit=5")
    assert resp.status_code == 200
...
async def test_crop_preset_partial_update(client):
    ...
    # Update only width
    resp = await client.put(f"/api/satellite/crop-presets/{pid}", json={"width": 500})
    assert resp.status_code == 200
