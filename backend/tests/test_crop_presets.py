...
class TestCropPresets:
    ...
    async def test_update_preset(self, client, db):
        ...
        db.add(p)
        await db.commit()
        resp = await client.put(f"/api/satellite/crop-presets/{p.id}", json={"name": "New"})
        assert resp.status_code == 200
...
class TestAnimationCreation:
    ...
    async def test_get_nonexistent_animation(self, client):
        resp = await client.get("/api/satellite/animations/fake-id")
        assert resp.status_code == 404

    async def test_delete_nonexistent_animation(self, client):
        resp = await client.delete("/api/satellite/animations/fake-id")
        assert resp.status_code == 404
