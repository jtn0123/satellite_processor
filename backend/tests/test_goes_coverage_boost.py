...
def test_record_s3_failure():
    with (
        patch("app.circuit_breaker.s3_circuit_breaker") as mock_cb,
...
class TestFetchFrames:
    ...
    def test_fetch_frames_capped(self, mock_val, mock_disk, mock_list, mock_s3, mock_proc, tmp_path):
        ...
        mock_list.return_value = [
            {"key": f"k{i}", "scan_time": datetime(2026, 1, 1, tzinfo=UTC), "size": 100} for i in range(10)
        ]
    ...
    def test_fetch_frames_with_failures(self, mock_val, mock_disk, mock_list, mock_s3, mock_proc, tmp_path):
        ...
        mock_list.return_value = [
            {"key": f"k{i}", "scan_time": datetime(2026, 1, 1, tzinfo=UTC), "size": 100} for i in range(3)
        ]
...
class TestLoadBandImages:
    def test_loads_bands_from_db(self, tmp_path):
        ...
        result = _load_band_images(
            session, ["C02", "C03", "C01"], "GOES-16", "FullDisk", datetime(2026, 1, 1, tzinfo=UTC)
        )
        assert len(result) == 3
    ...
    def test_missing_frame_returns_none(self):
        ...
        result = _load_band_images(session, ["C02"], "GOES-16", "FullDisk", datetime(2026, 1, 1, tzinfo=UTC))
        assert result == [None]
...
class TestGenerateComposite:
    ...
    def test_success(self, mock_db, mock_update, mock_pub, tmp_path):
        ...
        with (
            patch("app.tasks.composite_task._load_band_images", return_value=bands),
...
def test_list_available_s3_exception(mock_val, mock_s3, mock_retry):
    ...
    result = list_available(
        "GOES-16", "FullDisk", "C02", datetime(2026, 1, 1, tzinfo=UTC), datetime(2026, 1, 2, tzinfo=UTC)
    )
    assert result == []
