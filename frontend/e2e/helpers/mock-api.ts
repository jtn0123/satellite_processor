...
export async function handleApiRoute(route: Route): Promise<void> {
  ...
  if (url.includes('/api/health/changelog')) return void (await route.fulfill({ json: [] }));
  if (url.includes('/api/health')) return void (await route.fulfill({ json: { status: 'ok' } }));

  // Stats / notifications / settings
  if (url.includes('/api/stats'))
    return void (await route.fulfill({
      json: { total_images: 10, total_jobs: 5, active_jobs: 0, storage_used_mb: 256 },
    }));
  if (url.includes('/api/notifications')) return void (await route.fulfill({ json: [] }));
  ...
  if (url.includes('/api/satellite/catalog')) return void (await route.fulfill({ json: [] }));
  ...
  if (url.includes('/api/satellite/stats'))
    return void (await route.fulfill({
      json: { by_satellite: {}, by_band: {}, total_size: 0, total_frames: 0 },
    }));
  ...
  if (url.includes('/api/satellite/frames/preview-range'))
    return void (await route.fulfill({
      json: { frames: [], total_count: 0, capture_interval_minutes: 10 },
    }));
  if (url.includes('/api/satellite/frames'))
    return void (await route.fulfill({ json: { items: [], total: 0, page: 1, limit: 50 } }));
  if (url.includes('/api/satellite/collections')) return void (await route.fulfill({ json: [] }));
  if (url.includes('/api/satellite/tags')) return void (await route.fulfill({ json: [] }));
  if (url.includes('/api/satellite/crop-presets')) return void (await route.fulfill({ json: [] }));
  ...
  if (url.includes('/api/satellite/fetch-presets')) return void (await route.fulfill({ json: [] }));

  // Presets / jobs / images / system
  if (url.includes('/api/presets')) return void (await route.fulfill({ json: [] }));
