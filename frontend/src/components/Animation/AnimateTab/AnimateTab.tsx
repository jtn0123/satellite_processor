...
export default function AnimateTab() {
  ...
  const handleQuickHours = useCallback((hours: number) => { setDateRange(hours); }, [setDateRange]);
  ...
  const previewParams = useMemo(() => ({
    satellite: config.satellite, sector: config.sector, band: config.band,
    ...
    mutationFn: () => {
      if (sourceMode === 'collection' && collectionId) {
        const payload: Record<string, unknown> = {
          ...
          collection_id: collectionId, fps: config.fps, format: config.format,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <CreateAnimationForm
          ...
          collections={collections} handleQuickHours={handleQuickHours}
          previewEnabled={previewEnabled} previewData={previewData}
          previewLoading={previewLoading} previewError={previewError}
          onOpenSettings={() => setSettingsOpen(true)}
        />

        <div className="hidden lg:block space-y-4">
          <AnimationSettingsPanel config={config} captureIntervalMinutes={captureInterval} onChange={updateConfig} />
        ...
        <div className="bg-gray-50 dark:bg-slate-900 rounded-xl p-4 border border-gray-200 dark:border-slate-800">
          ...
          <div className="aspect-video bg-black rounded-lg overflow-hidden">
            {animationItems[0].format === 'gif' ? (
              <img src={`/api/download?path=${encodeURIComponent(animationItems[0].output_path)}`} alt={animationItems[0].name} className="w-full h-full object-contain" />
            ) : (
              <video src={`/api/download?path=${encodeURIComponent(animationItems[0].output_path)}`} controls loop className="w-full h-full object-contain">
                <track kind="captions" />
              </video>
            )}
          </div>
      ...
      >
        {generateMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : <Play className="w-5 h-5" />}
      ...
      <MobileSettingsPanel
        ...
        config={config} captureInterval={captureInterval}
        onChange={updateConfig} onLoadPreset={handleLoadPreset}
      />
