...
export default function AddToCollectionModal({
  ...
  return (
    <Modal onClose={onClose} ariaLabel="Add to Collection">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Add to Collection</h3>
        <button type="button" onClick={onClose} aria-label="Close collection modal">
      ...
      <div className="space-y-2">
        <label
          htmlFor="addcoll-existing-collection"
          className="text-sm text-gray-500 dark:text-slate-400"
        >
          Existing collection
        </label>
        <select
          id="addcoll-existing-collection"
        ...
        {selectedCollection && (
          <button
            type="button"
            onClick={() => addMutation.mutate(selectedCollection)}
      ...
      <div className="border-t border-gray-200 dark:border-slate-700 pt-4 space-y-2">
        <label
          htmlFor="addcoll-or-create-new"
          className="text-sm text-gray-500 dark:text-slate-400"
        >
          Or create new
        </label>
        <input
          id="addcoll-or-create-new"
          type="text"
          value={newName}
        ...
        {newName && (
          ...
          >
            {createAndAddMutation.isPending
              ? 'Creating...'
              : `Create & Add ${frameIds.length} frames`}
          </button>
        )}
      </div>
    </Modal>
