...
describe('DesktopBatchActions', () => {
  ...
  it('renders nothing when no items selected', () => {
    const { container } = render(<DesktopBatchActions {...baseProps} selectedIds={new Set()} />);
    expect(container.innerHTML).toBe('');
  });

  it('renders action buttons when items are selected', () => {
    render(<DesktopBatchActions {...baseProps} selectedIds={new Set(['a'])} />);
  ...
  it('shows Compare button when exactly 2 items selected', () => {
    render(<DesktopBatchActions {...baseProps} selectedIds={new Set(['a', 'b'])} />);
