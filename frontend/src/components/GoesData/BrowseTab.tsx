...
export default function BrowseTab() {
  ...
  const handleFrameClick = useCallback(
    (frame: GoesFrame, e: React.MouseEvent) => {
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-6">
        ...
        <FilterSidebar
          ...
          collectionsError={data.collectionsError}
          tags={data.tags}
        ...
        <div ref={pullRef} className="flex-1 space-y-4">
          ...
          <BrowseToolbar
            ...
            processMutation={data.processMutation}
          ...
          <div className="text-xs text-gray-400 dark:text-slate-500 min-h-[1.25rem]">
            {data.infiniteData ? (
              ...
              <span className="inline-block h-4 w-16 animate-pulse bg-gray-200 dark:bg-slate-700 rounded align-middle" />
            )}{' '}
          ...
          <FrameGridContent
            ...
            onAddToCollection={(f) => {
              setCollectionFrameIds([f.id]);
              setShowAddToCollection(true);
            }}
            onDelete={(f) => {
              setDeleteTargetIds([f.id]);
            }}
          />
          ...
          <BrowseModals
            ...
            onClosePreview={() => {
              ...
              globalThis.dispatchEvent(new CustomEvent('set-subview', { detail: null }));
            }}
            ...
            processingJobId={
              data.processMutation.isSuccess ? data.processMutation.data.job_id : null
            }
          />
        </div>

        <FloatingBatchBar
          selectedFrames={data.frames.filter((f) => selectedIds.has(f.id))}
          onCompare={() => {
            const sel = data.frames.filter((f) => selectedIds.has(f.id));
            if (sel.length === 2) setCompareFrames([sel[0], sel[1]]);
          }}
          onAnimate={() => {
            globalThis.dispatchEvent(new CustomEvent('switch-tab', { detail: 'animate' }));
            globalThis.dispatchEvent(
              new CustomEvent('animate-frames', { detail: [...selectedIds] }),
            );
          }}
          onTag={() => {
            setTagFrameIds([...selectedIds]);
            setShowTagModal(true);
          }}
          onAddToCollection={() => {
            setCollectionFrameIds([...selectedIds]);
            setShowAddToCollection(true);
          }}
          onDelete={() => {
            setDeleteTargetIds([...selectedIds]);
          ...
          onDownload={() => {
            data.frames.filter((f) => selectedIds.has(f.id)).forEach((f) => handleDownload(f));
          }}
          ...
          <ConfirmDialog
            title={
              deleteTargetIds.length === 1
                ? 'Delete 1 frame?'
                : `Delete ${deleteTargetIds.length} frames?`
            }
