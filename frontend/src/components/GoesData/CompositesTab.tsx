...
function getRecipeDescription(recipeId: string, satellite: string): string {
  const descs = isHimawariSatellite(satellite)
    ? HIMAWARI_RECIPE_DESCRIPTIONS
    : GOES_RECIPE_DESCRIPTIONS;
...
export default function CompositesTab() {
  ...
  const { data: recipes } = useQuery<CompositeRecipe[]>({
    ...
    queryFn: () =>
      api.get('/satellite/composite-recipes').then((r) => extractArray<CompositeRecipe>(r.data)),
  });
            ...
            >
              ...
              <div className="flex gap-1 mt-2">
                {recipe.bands.map((b) => (
                  <span
                    key={b}
                    className="px-1.5 py-0.5 text-[10px] bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-slate-300 rounded"
                  >
                    {b}
                  </span>
                ))}
      ...
      {selectedRecipe && (
        <div className="card p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label
                htmlFor="comp-capture-time"
                className="block text-sm font-medium text-gray-500 dark:text-slate-400 mb-1"
              >
                Capture Time
              </label>
              <input
                id="comp-capture-time"
                type="datetime-local"
                value={captureTime}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400 dark:text-slate-500">
                    {comp.satellite} · {comp.sector} ·{' '}
                    {new Date(comp.capture_time).toLocaleString()}
