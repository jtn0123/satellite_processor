...
export default function FetchTab() {
  ...
  return (
    <div className="space-y-4 pb-16">
      ...
      {step === 1 && (
        <WhatStep
          satellite={satellite} sector={sector} setSector={setSector}
          band={band} setBand={setBand} imageType={imageType} setImageType={setImageType}
          products={products} onBack={() => setStep(0)} onNext={() => setStep(2)}
        />
      )}

      {step === 2 && (
        <WhenStep
          satellite={satellite} sector={sector} band={band} imageType={imageType}
          startTime={startTime} setStartTime={setStartTime} endTime={endTime} setEndTime={setEndTime}
          currentAvail={currentAvail} sectorInfo={sectorInfo}
          onBack={() => setStep(1)} onConfirm={() => setShowConfirm(true)}
        />
      )}

      {showConfirm && (
        <ConfirmDialog
          satellite={satellite} sector={sector} imageType={imageType} band={band}
          estimate={estimate} isPending={fetchMutation.isPending}
          onConfirm={() => fetchMutation.mutate()} onCancel={() => setShowConfirm(false)}
        />
      )}
