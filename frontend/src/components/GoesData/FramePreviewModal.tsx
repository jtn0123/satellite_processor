...
export default function FramePreviewModal({
  ...
  const { data: cropPresets } = useQuery<CropPreset[]>({
    ...
    queryFn: () =>
      api.get('/satellite/crop-presets').then((r) => {
        ...
        {cropRect && (
          <div className="flex items-center gap-4 text-sm">
            ...
            <span className="text-gray-600 dark:text-slate-300">
              X: <span className="text-gray-900 dark:text-white font-mono">{cropRect.x}</span> Y:{' '}
              <span className="text-gray-900 dark:text-white font-mono">{cropRect.y}</span> W:{' '}
              <span className="text-gray-900 dark:text-white font-mono">{cropRect.w}</span> H:{' '}
              <span className="text-gray-900 dark:text-white font-mono">{cropRect.h}</span>
            </span>
            <button
              type="button"
