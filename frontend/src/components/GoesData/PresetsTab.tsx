...
export default function PresetsTab() {
  ...
  const createPreset = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['fetch-presets'] }); setShowCreate(false); resetForm(); },
  });

  const updatePreset = useMutation({
    mutationFn: ({ id, ...data }: { id: string } & Partial<typeof form>) => api.put(`/goes/fetch-presets/${id}`, data).then(r => r.data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['fetch-presets'] }); setEditingPreset(null); },
  });

  const deletePreset = useMutation({
    mutationFn: (id: string) => api.delete(`/goes/fetch-presets/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['fetch-presets'] }),
  });
  ...
  const createSchedule = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['fetch-schedules'] }); setShowScheduleCreate(false); },
  });

  const toggleSchedule = useMutation({
    mutationFn: (id: string) => api.post(`/goes/schedules/${id}/toggle`).then(r => r.data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['fetch-schedules'] }),
  });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
        ...
        <div className="space-y-3">
          {presets.length === 0 && <p className="text-slate-500 text-sm">No presets yet. Create one to save your fetch parameters.</p>}
          {presets.map(preset => (
            <div key={preset.id} className="flex items-center justify-between bg-slate-800 rounded-lg p-4">
              <div>
                <div className="font-medium">{preset.name}</div>
                <div className="text-sm text-slate-400">{preset.satellite} · {preset.sector} · {preset.band}</div>
                {preset.description && <div className="text-xs text-slate-500 mt-1">{preset.description}</div>}
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => runPreset.mutate(preset.id)} className="p-2 hover:bg-slate-700 rounded-lg text-green-400" title="Run Now">
                  <Play className="w-4 h-4" />
                </button>
                <button onClick={() => setEditingPreset(preset)} className="p-2 hover:bg-slate-700 rounded-lg text-slate-400" title="Edit">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => deletePreset.mutate(preset.id)} className="p-2 hover:bg-slate-700 rounded-lg text-red-400" title="Delete">
      ...
      <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
        ...
        {showScheduleCreate && (
          <div className="mb-4 bg-slate-800 rounded-lg p-4 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowScheduleCreate(false)} className="px-3 py-1.5 bg-slate-700 rounded-lg text-sm hover:bg-slate-600">
        ...
        <div className="space-y-3">
          {schedules.length === 0 && <p className="text-slate-500 text-sm">No schedules yet. Create a preset first, then schedule it.</p>}
          {schedules.map(sched => (
            <div key={sched.id} className="flex items-center justify-between bg-slate-800 rounded-lg p-4">
              <div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${sched.is_active ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-400'}`}
                >
                  {sched.is_active ? 'Active' : 'Inactive'}
                </button>
                <button onClick={() => deleteSchedule.mutate(sched.id)} className="p-2 hover:bg-slate-700 rounded-lg text-red-400">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
            </div>
  ...
  return (
    <div className="mb-4 bg-slate-800 rounded-lg p-4 space-y-3">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onCancel} className="px-3 py-1.5 bg-slate-700 rounded-lg text-sm hover:bg-slate-600">
          <X className="w-4 h-4 inline mr-1" /> Cancel
        </button>
      </div>
    </div>
