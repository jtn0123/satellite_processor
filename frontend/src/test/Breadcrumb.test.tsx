...
describe('Breadcrumb', () => {
  ...
  it('renders multiple segments', () => {
    render(<Breadcrumb segments={[{ label: 'Home', onClick: vi.fn() }, { label: 'Data' }]} />);
  ...
  it('clickable segments fire onClick', () => {
    const onClick = vi.fn();
    render(<Breadcrumb segments={[{ label: 'Home', onClick }, { label: 'Current' }]} />);
    fireEvent.click(screen.getByText('Home'));
    expect(onClick).toHaveBeenCalledOnce();
  });

  it('last segment is not clickable', () => {
    render(<Breadcrumb segments={[{ label: 'Home', onClick: vi.fn() }, { label: 'Current' }]} />);
    const last = screen.getByText('Current');
    expect(last.tagName).toBe('SPAN');
  });

  it('has aria-label', () => {
    render(<Breadcrumb segments={[{ label: 'A', onClick: vi.fn() }, { label: 'B' }]} />);
