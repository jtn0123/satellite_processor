...
const makeFrame = (id: string): GoesFrame =>
  ({
    ...
    thumbnail_url: `/api/satellite/frames/${id}/thumbnail`,
    file_size: 1000,
    width: 1920,
    height: 1080,
    tags: [],
...
describe('FloatingBatchBar', () => {
  ...
  it('shows count of selected frames', () => {
    render(
      <FloatingBatchBar selectedFrames={[makeFrame('1'), makeFrame('2')]} {...defaultHandlers} />,
    );
  ...
  it('shows Compare button only when exactly 2 frames selected', () => {
    const { rerender } = render(
      <FloatingBatchBar selectedFrames={[makeFrame('1')]} {...defaultHandlers} />,
    );
  ...
  it('calls onClear when clear clicked', () => {
    const onClear = vi.fn();
    render(
      <FloatingBatchBar selectedFrames={[makeFrame('1')]} {...defaultHandlers} onClear={onClear} />,
    );
  ...
  it('calls onTag when tag clicked', () => {
    const onTag = vi.fn();
    render(
      <FloatingBatchBar selectedFrames={[makeFrame('1')]} {...defaultHandlers} onTag={onTag} />,
    );
