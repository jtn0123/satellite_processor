...
describe('FrameCard', () => {
  it('renders in grid mode with buttons', () => {
    const onClick = vi.fn();
    render(
      <FrameCard frame={frame as never} isSelected={false} onClick={onClick} viewMode="grid" />,
    );
  ...
  it('renders in list mode', () => {
    const onClick = vi.fn();
    render(
      <FrameCard frame={frame as never} isSelected={false} onClick={onClick} viewMode="list" />,
    );
    const buttons = screen.getAllByRole('button');
    expect(buttons.length).toBeGreaterThan(0);
  });
  ...
  it('shows selected indicator when isSelected', () => {
    render(
      <FrameCard frame={frame as never} isSelected={true} onClick={vi.fn()} viewMode="grid" />,
    );
  ...
  it('displays satellite and band badges', () => {
    render(
      <FrameCard frame={frame as never} isSelected={false} onClick={vi.fn()} viewMode="grid" />,
    );
  ...
  it('displays file size in list mode', () => {
    render(
      <FrameCard frame={frame as never} isSelected={false} onClick={vi.fn()} viewMode="list" />,
    );
  ...
  it('all interactive elements have min 44px touch targets', () => {
    ...
    buttons.forEach((btn) => {
      ...
      const hasMinSize =
        style.includes('min-h-[44px]') ||
        style.includes('min-w-[44px]') ||
        style.includes('aspect-video');
