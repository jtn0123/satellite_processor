...
describe('Himawari UX: Schedule Auto-fetch button', () => {
  it('shows button in empty state', async () => {
    ...
    expect(screen.getByTestId('himawari-schedule-auto-fetch')).toHaveTextContent(
      'Schedule Auto-fetch',
    );
  });
  ...
  it('calls preset and schedule endpoints on click', async () => {
    ...
    await switchToHimawari();
    await act(async () => {
      fireEvent.click(screen.getByTestId('himawari-schedule-auto-fetch'));
    });

    await waitFor(() => {
      const presetCall = mockedApi.post.mock.calls.find(
        (c: string[]) => c[0] === '/satellite/fetch-presets',
      );
      expect(presetCall).toBeTruthy();
      expect(presetCall[1]).toEqual(
        expect.objectContaining({ satellite: 'Himawari-9', sector: 'FLDK' }),
      );
    });
    await waitFor(() => {
      const scheduleCall = mockedApi.post.mock.calls.find(
        (c: string[]) => c[0] === '/satellite/schedules',
      );
      expect(scheduleCall).toBeTruthy();
      expect(scheduleCall[1]).toEqual(
        expect.objectContaining({ preset_id: 'preset-1', interval_minutes: 10, is_active: true }),
      );
    });
  });
...
describe('Himawari UX: Sector cadence labels', () => {
  it('HIMAWARI_SECTORS have cadence descriptions', () => {
    ...
    expect(sectors[0]).toEqual(
      expect.objectContaining({ id: 'FLDK', description: '10-min cadence' }),
    );
    expect(sectors[1]).toEqual(
      expect.objectContaining({ id: 'Japan', description: '2.5-min cadence' }),
    );
    expect(sectors[2]).toEqual(
      expect.objectContaining({ id: 'Target', description: '2.5-min cadence' }),
    );
  });
...
describe('Himawari UX: S3 availability indicator', () => {
  it('shows "No recent data found on S3" when no catalog data', async () => {
    ...
    await switchToHimawari();
    await waitFor(() => {
      expect(screen.getByTestId('s3-availability')).toHaveTextContent('No recent data found on S3');
    });
  });
  ...
  it('shows time-ago when S3 data available', async () => {
    ...
    await switchToHimawari();
    await waitFor(() => {
      expect(screen.getByTestId('s3-availability')).toHaveTextContent(/Latest on S3: \d+ min ago/);
    });
  });
  ...
  it('makes catalog/latest API call for Himawari', async () => {
    ...
    await waitFor(() => {
      const calls = mockedApi.get.mock.calls.filter(
        (c: unknown[]) =>
          c[0] === '/satellite/catalog/latest' &&
          (c[1] as Record<string, Record<string, string>> | undefined)?.params?.satellite ===
            'Himawari-9',
      );
