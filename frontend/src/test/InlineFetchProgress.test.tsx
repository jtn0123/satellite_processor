...
describe('InlineFetchProgress', () => {
  it('shows progress bar with correct percentage', () => {
    render(
      <InlineFetchProgress
        job={{ id: '1', status: 'running', progress: 42, status_message: 'Downloading...' }}
      />,
    );
  ...
  it('displays status message', () => {
    render(
      <InlineFetchProgress
        job={{ id: '1', status: 'running', progress: 30, status_message: 'Processing tiles...' }}
      />,
    );
  ...
  it('shows "Fetching…" when no status_message', () => {
    render(
      <InlineFetchProgress job={{ id: '1', status: 'running', progress: 0, status_message: '' }} />,
    );
  ...
  it('shows complete state', () => {
    render(
      <InlineFetchProgress
        job={{ id: '1', status: 'completed', progress: 100, status_message: '' }}
      />,
    );
  ...
  it('shows failed state', () => {
    render(
      <InlineFetchProgress
        job={{ id: '1', status: 'failed', progress: 50, status_message: 'Error' }}
      />,
    );
  ...
  it('renders progress bar width matching progress', () => {
    const { container } = render(
      <InlineFetchProgress
        job={{ id: '1', status: 'running', progress: 65, status_message: 'Working...' }}
      />,
    );
