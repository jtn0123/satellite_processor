...
describe('InlineFetchProgress', () => {
    ...
    const { container } = render(
      <InlineFetchProgress
        job={{
          ...
          status: 'processing',
          progress: 45,
          status_message: 'Downloading band C02',
        }}
      />,
    );
  ...
  it('shows "Fetching…" when no status_message', () => {
    render(
      <InlineFetchProgress
        job={{ id: 'j1', status: 'processing', progress: 10, status_message: '' }}
      />,
    );
  ...
  it('shows success for completed job', () => {
    render(
      <InlineFetchProgress
        job={{ id: 'j1', status: 'completed', progress: 100, status_message: '' }}
      />,
    );
  ...
  it('shows failure for failed job', () => {
    render(
      <InlineFetchProgress
        job={{ id: 'j1', status: 'failed', progress: 50, status_message: '' }}
      />,
    );
  ...
  it('progress bar width matches job progress', () => {
    const { container } = render(
      <InlineFetchProgress
        job={{ id: 'j1', status: 'processing', progress: 75, status_message: 'Working' }}
      />,
    );
