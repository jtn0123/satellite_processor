...
def mock_directories(tmp_path):
    """Create mock input/output directories with test frames"""
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir(parents=True)
    output_dir.mkdir(parents=True)

    # Create test frames with proper naming
    for i in range(5):
...
class TestVideoOptions:
    ...
    def test_encoder_options(self, video_options):
        ...
        encoder_options = [
            video_options.encoder_combo.itemText(i)
    ...
    def test_invalid_interpolation_factor(self, video_options, qtbot):
        ...
        assert "Interpolation factor must be between 2 and 8 for High quality" in str(
            exc_info.value
        )
    ...
    def test_hardware_selection_affects_encoder_options(self, video_options, qtbot):
        ...
        assert (
            video_options.encoder_combo.count() == 5
        )  # Updated expected count to match actual
        encoder_options = [
            video_options.encoder_combo.itemText(i)
            for i in range(video_options.encoder_combo.count())
        ]
    ...
    def test_interpolation_function_called_with_correct_params(self, video_options):
        ...
        with patch(
            "satellite_processor.core.image_operations.ImageOperations.process_image"
        ) as mock_process:
            video_options.enable_interpolation.setChecked(True)
            video_options.quality_combo.setCurrentText("High")
    ...
    def test_ai_interpolation_methods(self, video_options):
        ...
        with patch(
            ...
            result = processor.process_image(
                "test.png",
                {**video_options.get_options(), "interpolation_method": "RIFE"},
            )
    ...
    def test_interpolation_edge_cases(self, video_options):
        ...
        assert "Interpolation factor must be between 2 and 4 for Low quality" in str(
            exc_info.value
        )
    ...
    def test_bit_rate_settings(self, video_options):
        ...
        with patch(
            "satellite_processor.core.video_handler.VideoHandler.set_bitrate"
        ) as mock_bitrate:
    ...
    def test_bitrate_validation(self, video_options):
        ...
        video_options.testing = True
        with pytest.raises(
            ValueError, match="Bitrate must be between 100 and 10000 kbps."
        ):
    ...
    def test_quality_dependent_interpolation(self, video_options):
        ...
        with pytest.raises(
            ValueError,
            match="Interpolation factor must be between 2 and 6 for Medium quality",
        ):
    ...
    def test_transcoding_process(self, mock_transcode, video_options):
        ...
        video_handler.transcode_video(
            "/path/to/input/video", "/path/to/output/video", options
        )

        mock_transcode.assert_called_with(
            "/path/to/input/video", "/path/to/output/video", options
        )
    ...
    def test_transcoding_formats(self, video_options):
        ...
        format_options = [
            video_options.transcoding_format_combo.itemText(i)
            for i in range(video_options.transcoding_format_combo.count())
        ]
    ...
    def test_transcoding_disabled(
        self, mock_run, video_options, mock_path_exists, mock_path_is_dir, tmp_path
    ):
        ...
        # Use temporary directory instead of fixed drive path
        input_dir = tmp_path / "input"
        output_path = tmp_path / "output.mp4"
        input_dir.mkdir(parents=True, exist_ok=True)  # Add exist_ok=True
        output_path.parent.mkdir(parents=True, exist_ok=True)  # Add exist_ok=True

        # Create a test frame
        frame_path = input_dir / "frame0000.png"
        frame = np.ones((100, 100, 3), dtype=np.uint8) * 128
        cv2.imwrite(str(frame_path), frame)

        with patch("subprocess.run", return_value=MagicMock(returncode=0)) as mock_run:
            success = video_handler.create_video(
                str(input_dir), str(output_path), options
            )
    ...
    def test_frame_transition_smoothness(self, video_options):
        ...
        def mock_process_image(input_frames, options):
            ...
            for i in range(1, factor + 1):
                alpha = i / (factor + 1)
                interpolated_frame = (
                    frame_start * (1 - alpha) + frame_end * alpha
                ).astype(np.uint8)
        ...
        with patch(
            "satellite_processor.core.image_operations.ImageOperations.process_image",
            side_effect=mock_process_image,
        ) as mock_process:
            video_options.enable_interpolation.setChecked(True)
            video_options.quality_combo.setCurrentText("High")
            ...
            for idx, frame in enumerate(interpolated, start=1):
                ...
                expected_frame = (
                    frame_start * (1 - expected_alpha) + frame_end * expected_alpha
                ).astype(np.uint8)
                assert np.array_equal(
                    frame, expected_frame
                ), f"Interpolated frame {idx} does not match expected values."
...
class TestVideoCreation:
    ...
    def test_video_creation_basic(self, video_handler, mock_directories):
        ...
        with patch("psutil.Process", return_value=process_mock) as proc_mock, patch(
            "subprocess.run", return_value=MagicMock(returncode=0)
        ), patch("pathlib.Path.exists", return_value=True), patch(
            "pathlib.Path.is_dir", return_value=True
        ), patch(
            "pathlib.Path.glob",
            return_value=[Path(input_dir) / f"frame_{i:04d}.png" for i in range(3)],
        ):
            ...
            success = video_handler.create_video(
                input_dir, Path(output_dir) / "output.mp4", options
            )
            assert success is True
            ...
            ), "Memory monitoring was not performed"
    ...
    def test_ffmpeg_command_generation(self, video_options, mock_ffmpeg):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Use unified patching approach
            with patch.multiple(
                Path,
                exists=MagicMock(return_value=True),
                is_dir=MagicMock(return_value=True),
                glob=MagicMock(return_value=sorted(frame_paths)),
            ):

                # Create video and verify command
                output_path = output_dir / "output.mp4"
                video_handler.create_video(input_dir, output_path, options)
                ...
                # Check essential command components
                assert "-framerate" in cmd_str
                assert "frame%04d.png" in cmd_str or "frames.txt" in cmd_str
    ...
    def test_ffmpeg_encoder_selection(
        ...
        for ui_encoder, ffmpeg_encoder in encoder_mappings.items():
            ...
            video_options.hardware_combo.setCurrentText(
                f"{hardware} GPU" if hardware == "NVIDIA" else hardware
            )
            ...
            assert (
                options["encoder"] == ui_encoder
            ), f"Encoder {ui_encoder} not set correctly in options"
            ...
            with patch(
                "subprocess.run", return_value=MagicMock(returncode=0)
            ) as mock_run, patch("pathlib.Path.exists", return_value=True), patch(
                "pathlib.Path.is_dir", return_value=True
            ), patch(
                "pathlib.Path.glob", return_value=[Path(input_dir) / "frame0000.png"]
            ):

                video_handler.create_video(
                    input_dir, Path(output_dir) / "video.mp4", options
                )
                ...
                assert (
                    expected_encoder in command_str
                ), f"Expected encoder {expected_encoder} not found in command: {command_str}"

    def test_interpolation_frame_count(
        self, video_options, mock_path_exists, mock_path_is_dir
    ):
    ...
    def test_ffmpeg_error_handling(self, video_options, mock_directories):
        ...
        with patch("pathlib.Path.glob") as mock_glob, patch(
            ...
            with pytest.raises(
                RuntimeError, match="FFmpeg error: FFmpeg error message"
            ):
                video_handler.create_video(
                    input_dir,
    ...
    def test_custom_ffmpeg_options(
        self, video_options, mock_path_exists, mock_path_is_dir
    ):
    ...
    def test_interpolation_disabled(self, video_options):
        ...
        with patch.object(
            processor, "process_image", return_value=np.zeros((100, 100, 3))
    ...
    def test_video_creation_with_transcoding(
        self, video_options, mock_ffmpeg, mock_directories
    ):
        ...
        with patch.object(
            VideoHandler, "transcode_video", return_value=True
        ) as mock_transcode:
            video_handler.create_video(
                input_dir, Path(output_dir) / "output.mp4", options
            )
    ...
    def test_video_creation_with_different_frame_rates(
        self, video_options, mock_ffmpeg, mock_directories
    ):
    ...
    def test_video_handler_threading(self, video_options, mock_directories):
        ...
        # Create actual test frames with content
        frame_files = []
        frame = np.ones((100, 100, 3), dtype=np.uint8) * 128
        for i in range(5):
        ...
        # Use CPU encoding to avoid GPU-related issues
        options = video_options.get_options()
        ...
        # Use an Event for synchronization and a list for error collection
        completion_event = threading.Event()
        thread_exception = []

        def thread_task():
            try:
                result = video_handler.create_video(
                    input_dir, Path(output_dir) / "video.mp4", options
                )
                if not result:
                    thread_exception.append(RuntimeError("Video creation failed"))
            except Exception as e:
                thread_exception.append(e)
            finally:
                completion_event.set()

        # Start thread with proper mocking
        with patch.multiple(
            Path,
            exists=MagicMock(return_value=True),
            is_dir=MagicMock(return_value=True),
            glob=MagicMock(return_value=frame_files),
            ...
            # Wait with timeout
            if not completion_event.wait(timeout=5):
                pytest.fail("Thread did not complete in time")

            # Check for exceptions
            if thread_exception:
                pytest.fail(f"Thread encountered an error: {thread_exception[0]}")
    ...
    def test_frame_rate_synchronization(self, video_options, mock_directories):
        ...
        with patch(
            "pathlib.Path.glob",
            return_value=[Path(input_dir) / f"frame{i:04d}.png" for i in range(4)],
    ...
    def test_hardware_acceleration_fallback(self, video_options, mock_directories):
        ...
        with patch("pathlib.Path.glob", return_value=[frame_path]), patch(
            "subprocess.run"
        ) as mock_run:
            mock_run.side_effect = [
                subprocess.CalledProcessError(
                    1, ["ffmpeg"], stderr=b"Cannot use NVENC"
                ),
    ...
    def test_error_handling_corrupted_frames(
        self, video_options, mock_directories, caplog
    ):
        ...
        with patch(
            ...
            mock_run.side_effect = subprocess.CalledProcessError(
                1, ["ffmpeg"], stderr=b"Error reading corrupted frame"
            )
            ...
            assert any(
                "Error reading corrupted frame" in record.message
    ...
    def test_timestamp_directory_frame_handling(self, video_options, mock_directories):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        timestamp_dir = (
            Path(output_dir) / f"processed_{timestamp}/03_timestamp_{timestamp}"
        )
        timestamp_dir.mkdir(parents=True, exist_ok=True)
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            ...
            "pathlib.Path.resolve", return_value=timestamp_dir
        ):

            with pytest.raises(RuntimeError) as exc_info:
                video_handler.create_video(
                    timestamp_dir, Path(output_dir) / "output.mp4", options
                )
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "pathlib.Path.is_dir", return_value=True
        ), patch("pathlib.Path.glob", return_value=test_frames), patch(
            "subprocess.run", return_value=MagicMock(returncode=0)
        ) as mock_run:
    ...
    def test_parallel_processing_output_path(self, video_options, mock_directories):
        ...
        with patch("pathlib.Path.glob", return_value=test_frames), patch(
            "subprocess.run", return_value=MagicMock(returncode=0)
        ) as mock_run, patch("pathlib.Path.exists", return_value=True), patch(
            "pathlib.Path.is_dir", return_value=True
        ):
            ...
            success = video_handler.create_video(
                stage_dirs["timestamp"], process_dir / "final_video.mp4", options
            )
            ...
            input_pattern = (
                str(stage_dirs["timestamp"]).replace("\\", "/") + "/frame%04d.png"
            )
            assert input_pattern in cmd_str.replace(
                "\\", "/"
            ), f"Expected pattern {input_pattern} not found in command: {cmd_str}"
    ...
    def test_timestamped_directory_structure(self, video_options, mock_directories):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            ...
            with pytest.raises(RuntimeError) as exc_info:
                video_handler.create_video(
                    stage_dir, process_dir / "output.mp4", options
                )
        ...
        with patch(
            "pathlib.Path.glob",
            return_value=[stage_dir / f"frame{i:04d}.png" for i in range(5)],
        ), patch("subprocess.run", return_value=MagicMock(returncode=0)) as mock_run:
    ...
    def test_output_file_validation(self, video_options, mock_directories):
        ...
        for invalid_path in ["", "invalid/path/test.mp4", output_dir / "test.invalid"]:
            try:
                video_handler.create_video(
                    input_dir, invalid_path, video_options.get_options()
                )
    ...
    def test_framerate_conversion(self, video_options, mock_directories):
        ...
        for input_fps, output_fps in test_cases:
            ...
            with patch(
                ...
                video_handler = VideoHandler()
                video_handler.create_video(
                    input_dir, output_dir / f"test_{output_fps}fps.mp4", options
                )
    ...
    def test_concurrent_video_creation(self, video_options, mock_directories):
        ...
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            first_creation = video_handler.create_video(
                input_dir, output_dir / "first.mp4", options
            )
            assert first_creation is True

            # Attempt second video creation while first is running
            video_handler._is_processing = True
            second_creation = video_handler.create_video(
                input_dir, output_dir / "second.mp4", options
            )
    ...
    def test_resource_monitoring(self, video_options, mock_directories):
        ...
        with patch("psutil.Process", return_value=process_mock) as proc_mock, patch(
            "subprocess.run", return_value=MagicMock(returncode=0)
        ) as ffmpeg_mock, patch("pathlib.Path.exists", return_value=True), patch(
            "pathlib.Path.is_dir", return_value=True
        ), patch(
            "pathlib.Path.glob", return_value=[Path(input_dir) / "frame0000.png"]
        ):
            ...
            # Create video
            success = video_handler.create_video(
                input_dir, output_dir / "output.mp4", options
            )
            ...
            assert (
                process_mock.cpu_percent.call_count > 0
            ), "CPU monitoring was not called"
            assert (
                process_mock.memory_info.call_count > 0
            ), "Memory monitoring was not called"
    ...
    def test_video_creation_resume(self, video_options, mock_directories):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.side_effect = [
                subprocess.CalledProcessError(
                    1, ["ffmpeg"], stderr=b"Temporary failure"
                ),
                MagicMock(returncode=0),
            ]
    ...
    def test_progress_cancellation(self, video_options, mock_directories):
        ...
        def mock_run(*args, **kwargs):
            ...
            raise subprocess.CalledProcessError(
                1, cmd="ffmpeg", stderr=b"Cancelled by user"
            )
    ...
    def test_input_frame_validation(self, video_options, mock_directories):
        ...
        with patch("pathlib.Path.glob", return_value=frames), patch(
    ...
    def test_no_frame_files_error(self, video_handler, mock_directories, caplog):
        """Test error handling when no frame files are found."""
        input_dir, output_dir = mock_directories
        ...
        assert any(
            "No frame files found" in msg for msg in error_messages
        ), "Error log not found"
    ...
    def test_hardware_specific_filters(self, video_options, mock_directories):
        ...
        for hardware, (encoder, expected_filter) in filter_mappings.items():
            with patch(
                ...
                video_options.encoder_combo.setCurrentText(
                    "H.264"
                )  # Use standard H.264
                options = video_options.get_options()
        ...
        for hw_type in hardware_types:
            with patch("subprocess.run") as mock_run:
                ...
                mock_run.return_value = MagicMock(
                    returncode=0, stdout=f"Supported hardware accelerators: {hw_type}"
                )
...
class TestNetworkSupport:
    ...
    def test_timestamped_directory_processing(
        self, video_options, mock_directories, tmp_path
    ):
        ...
        # Mock temporary directory creation and ensure it exists
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            ...
            success = video_handler.create_video(
                timestamp_dir, process_dir / "output.mp4", options
            )
            assert success is True
            mock_run.assert_called_once()
...
def create_test_frames(directory: Path, count: int = 5) -> List[Path]:
    ...
    def build_ffmpeg_command(self, input_path, output_path, options):
        ...
        temp_dir = None
        try:
            # Create a temporary directory for the files list
            temp_dir = Path(tempfile.mkdtemp())
            temp_list_file = temp_dir / "files.txt"
            ...
            if temp_dir is not None and isinstance(temp_dir, Path):
                try:
