...
class SettingsDialog(QDialog):
    ...
    def browse_sanchez(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Sanchez Executable", filter="Executable files (*.exe)"
        )
        if path:
            exe_path = Path(path)
            if exe_path.name.lower() != "sanchez.exe":
                QMessageBox.warning(
                    self, "Invalid File", "Please select the Sanchez.exe executable."
                )
                return
    ...
    def browse_underlay(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Underlay Image", filter="Image files (*.jpg *.png)"
        )
        if path:
    ...
    def save_settings(self):
        ...
        try:
            ...
            if not os.path.exists(sanchez_path):
                QMessageBox.warning(
                    self, "Invalid Path", "Sanchez executable not found."
                )
                return
    ...
    def accept(self):
        ...
        try:
            ...
            input_dir = (
                Path(self.input_dir.text()).resolve()
                if self.input_dir.text()
                else Path()
            )
            output_dir = (
                Path(self.output_dir.text()).resolve()
            ...
            # Verify paths exist
            for path, name in [
                (sanchez_path, "Sanchez executable"),
                (underlay_path, "Underlay image"),
            ]:
                if not path.exists():
                    QMessageBox.warning(
                        self, "Invalid Path", f"{name} not found at: {path}"
                    )
                    return
