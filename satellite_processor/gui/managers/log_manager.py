...
from ...utils.url_handler import (
    create_file_url,
    create_link_data,
)  # Add create_link_data to imports
import os
...
class LogManager(QObject):
    ...
    def _handle_success_message(self, message: str) -> None:
        ...
        if message != self._last_success:
            ...
            formatted_message = (
                f'<div style="color: #2ecc71; margin: 5px 0;">{message}</div>'
            )
    ...
    def _normalize_network_path(self, file_path: Path) -> Path:
        ...
        try:
            ...
            if path_str.startswith(r"\\") and WindowsPath is not None:  # UNC path
    ...
    def log_completion(self, file_path: Optional[Path] = None):
        ...
        try:
            if file_path and file_path.exists():
                ...
                self.link_received.emit(
                    link_html
                )  # Use link_received for the HTML link
