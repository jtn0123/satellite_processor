...
class ProcessingOptionsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.hardware_combo.addItems(
            ["CPU", "NVIDIA GPU (NVENC)", "AMD GPU (AMF)", "Intel GPU (QSV)"]
        )
        ...
        self.sanchez_method.addItems(["Standard", "Enhanced", "Natural", "Fire"])
        ...
        self.interp_method.addItems(
            ["Motion Compensated (MCI)", "Bidirectional", "Advanced Optical Flow"]
        )

        self.interp_quality = QComboBox()
        self.interp_quality.addItems(
            ["High (Best Quality)", "Medium (Balanced)", "Low (Faster)"]
        )
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.codec_combo.setCurrentText(
                options.get("codec", "H.264 (Maximum Compatibility)")
            )
            ...
            # Sanchez options
            self.enable_false_color.setChecked(
                options.get("false_color_enabled", False)
            )
            self.sanchez_method.setCurrentText(
                options.get("false_color_method", "Standard")
            )

            # Interpolation options
            self.enable_interpolation.setChecked(
                options.get("interpolation_enabled", False)
            )
            self.interp_method.setCurrentText(
                options.get("interpolation_method", "Linear")
            )
    ...
    def load_options(self, options: dict):
        ...
        try:
            ...
            self.interp_method.setCurrentText(
                options.get("interpolation_method", "Motion Compensated (MCI)")
            )
            ...
            self.interp_quality.setCurrentText(
                quality_map.get(quality, "Medium (Balanced)")
            )
    ...
    def _on_false_color_toggled(self, enabled: bool):
        ...
        if enabled:
            self.logger.info(
                f"False color enabled with method: {self.sanchez_method.currentText()}"
            )
