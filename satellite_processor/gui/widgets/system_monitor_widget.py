...
class GPUMonitor(QObject):
    ...
    def get_gpu_stats(self):
        ...
        if INTEL_AVAILABLE and self.wmi_interface:
            try:
                perf_data = self.wmi_interface.Win32_PerfFormattedData_GPUPerformanceCounters_GPUEngine(
                    [], timeout_ms=100
                )
                total = sum(
                    int(gpu.UtilizationPercentage)
                    for gpu in perf_data
                    if hasattr(gpu, "UtilizationPercentage")
                )
                count = sum(
                    1 for gpu in perf_data if hasattr(gpu, "UtilizationPercentage")
                )
                if count > 0:
...
class SystemMonitorWidget(QWidget):  # Just rename the class from ResourceMonitorWidget
    ...
    def __init__(self, parent=None):
        ...
        self.last_values = {"cpu": 0, "ram": 0, "upload": 0, "download": 0, "nvidia": 0}

        self.last_values.update({"nvidia_gpu": 0, "intel_gpu": 0})
        ...
        self.value_history.update({"gpu_nvidia": [], "gpu_intel": []})
    ...
    def _apply_bar_style(self, bar, value: float, key: str):
        ...
        if key in self.last_values:
            ...
            if value > current:
                ...
                decayed_value = current + (value - current) * (
    ...
    def update_stats(self):
        ...
        try:
            ...
            self._update_cpu_ram(
                psutil.cpu_percent(interval=None), psutil.virtual_memory().percent
            )
